...
signed main(int argc, char* argv[])
{
    ...
    if (listen(listening_socket, 10) < 0) {
        ...
        if (conn_channel == 0)
        {
            ...
            while(!success)
            {
                GREEN << "Starting Connection Channel" ; RESET2;
                ...
                else
                {
                    ...
                    {
                        WHITE << "Paused for SIGCHLD or SIGINT" ; RESET2;
                        pause();
                        if (success)                                    //  Caught SIGCHLD with EXIT_SUCCESS
                        {
                            GREEN << "PROGRAM TERMINATION SUCCESSFUL" ; RESET2;
                        }
                        else if (finished)                              //  Caught SIGCHLD with EXIT_FAILURE
                        {
                            GREEN << "PROGRAM FAILED" ; RESET2;
                        }
                        else                                            //  Caught SIGINT meant for PARENT
                        {
                            WHITE << "Paused for SIGCHLD or SIGMODIFY" ; RESET2;
                            pause();
                            ...
                            {
                                WHITE << "Code Modification has started" << endl ;
                                ...
                                else                //  Caught SIGCHLD with EXIT_FAILURE (Common)
                                {
                                    ...
                                    WHITE << "Received " << *offset << " from Data Channel" ; RESET2;
                                }
                                delete[] offset_Str;
                            }
                            if (modify == 2)   //  Caught SIGMODIFY Second Time (Common)
                            {
                                WHITE << "Code Modification has ended" << endl ; RESET1
                            }
                        }
                    }
                }
        ...
        else
        {
            while(!complete)
            {
                ...
                if (interrupt)
                {
                    WHITE << "Modification Started" ; RESET2;
                    ...
                    WHITE << "Waiting Inside for SIGCHLD or SIGINT" ; RESET2;
                    pause();
                    if (interrupt)
                    {
            ...
            complete = 0;
            GREEN << "TERMINATION of CHILD" ; RESET2;
        }
